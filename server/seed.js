...
if (!data.coops.find((c) => c.slug === COOP_SLUG)) {
  data.coops.push({
    id:            uuid(),
    slug:          COOP_SLUG,
    ...
    zip_codes:     ['84401','84403','84404','84405','84414','84015','84010','84070','84094','84020','84043','84057','84058','84601','84604','84606','84102','84103','84104','84105','84106','84107','84108','84111','84115','84116','84117','84119','84120','84121','84123','84124'],
    color:         '#2d8b8b',
    logo_letter:   'W',
    active:        true,
...
let partner = data.partners.find((p) => p.email === PARTNER_EMAIL);
if (!partner) {
  partner = {
    id:           uuid(),
...
const PARTNER2_EMAIL = 'ops@phonecash.demo';
if (!data.partners.find((p) => p.email === PARTNER2_EMAIL)) {
  data.partners.push({
    id:           uuid(),
...
for (const d of DRIVERS) {
  if (data.drivers.find((x) => x.email === d.email)) continue;
  data.drivers.push({
    id:           uuid(),
    coop_slug:    COOP_SLUG,
    name:         d.name,
  ...
  console.log('✓  Driver:', d.email, '/ driver1234');
}

// ── 4. Seed sample orders ─────────────────────────────────────────────────────
const assignedDriver = data.drivers.find((d) => d.email === 'sam.driver@oddcoop.demo');
const ORDERS = [
...
for (const o of ORDERS) {
  if (data.orders.find((x) => x.external_ref === o.external_ref)) continue;
  data.orders.push({
    id:           uuid(),
...
console.log('  Partner login:  ops@phonecash.demo / demo1234');
...
console.log('');
console.log('  Coop network:   GET /api/coop/network');
console.log('  Territory ZIP:  GET /api/coop/territory/84101');
console.log('  Onboarding:     POST /api/coop/register');
console.log('  Join page:      http://localhost:3847/join');
console.log('');
